...
3. Keep the defaults: **96 kHz, 90% amplitude, Reversed polarity, Standard Note Off, 0 idle bits**.
...
6. Use **PANIC** or **Command–.** if notes get stuck. It cancels tests, drops queued traffic, finishes the current UART byte, then sends CC123, CC120, and pitch-bend center on all 16 channels. Stop and Quit send Panic before releasing audio.
...
- Amplitude, polarity, note-off mode, and idle spacing apply while running. Stop output before changing device or sample rate. Note-off mode is latched when each message begins.
...
CoreMIDI host timestamps are retained. Bytes are not started before their timestamp; late/immediate input starts as soon as the audio callback can consume it. FIFO arrival order wins when callers submit conflicting/out-of-order timestamps; this is not a sequencer that reorders events. Like any MIDI UART, traffic above 3,125 bytes/second accumulates latency. Audio hardware buffering adds latency, and wall-clock test-generator scheduling is not a hard realtime clock.
...
- Identical output across irregular audio buffer splits, silence, future timestamps, polarity, amplitude, and idle spacing.
